...
* **ARINT products** represents different "ready made products" which have a complete defined functionality. *These are completely eparated products from commercial point of view*, but some of them can be included in different ARINT licensing editions as "embedded" or as "price value pack" at first license
...
* (USLAS) can be included in first *ARINT license* as being an API product able to assure a consistent serialization for different business objects (ie, own invoices, contracts, etc)
* -#NOTE reserved for future items
