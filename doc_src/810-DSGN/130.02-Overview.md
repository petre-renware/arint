...
**ALPHAREN Core Integrator** (aka **`ARINT`** or **`arint`**) system is a framework product for _automation_, _integrations_ and _interoperability_ aimed to connect _distributed systems_ or _data sources_ and to build _API oriented_, _middleware_, _frontend_ and _backend_ applications (microservices style). Product is available as *distinct software* or as *ready to run appliance* (including also some built-in components such as an internal database for business operations).
