...
<small markdown>These components are considered "auxiliary" because in most cases they are not needed anymore and in consequence are not part of installation process (to not induce useless overhead at system resources level).</small>

* `[0 to 1]` *internal DNS / dynamic DNS* which is a standard DNS inside the *ARCLST*. No ARINT component will use it but will allow for a "nice" outside LAN addressing of *ARCLST* components when customer explicitly wants and needs that, otherwise ARINT has a proxy and balacer included whixh assure its addressing as sysyem and for business purposes
...
Normally these components are left at router glance as being normal included functionalitatile in enterprise / business router.
