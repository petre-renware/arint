...
An *ARCLST* can be linked with other ARCLST nodes and can work in mode, ie as *active-active* or *active-pasive* at the administrator option to configure system.

An *ARCLST* must be located in the same LAN (all machines inside should be able to communicate between them at IP level).
